	...
	<div>
		<div class="node creole">
			<div class="answer formatted">
				...
				<p>
					...
					mode, which can be enabled with the
					<code>
						<a href="#-innobackupex">--innobackupex</a>
					</code>
					option.
				</p>
				<p>
					...
					operations in the target directory are not ready for use on the
				...
				<p>
					...
					rejects the new data as corrupt. Running Mariabackup with the
					<code>--prepare</code>
					command readies the data so you can restore it to MariaDB Server.
					When working with incremental backups, you need to use the
					<code>--prepare</code>
					command and the
					<code>
						<a href="#-incremental-dir">--incremental-dir</a>
					</code>
					option to update the base backup with the deltas from an
					incremental backup.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --innobackupex --apply-log
</pre>
				<p>
					Once the backup is ready, you can use the
					<code>
						<a href="#-copy-back">--copy-back</a>
					</code>
					or the
					<code>
						<a href="#-move-back">--move-back</a>
					</code>
				...
				<p>
					...
					recovery will be ignored. This option is used with <a
						href="../incremental-backup-and-restore-with-mariabackup/index">incremental
						backups</a>.
				</p>
				<p>
					This option is only supported in <a
						href="../what-is-mariadb-101/index">MariaDB 10.1</a>. In <a
						href="../what-is-mariadb-102/index">MariaDB 10.2</a> and later,
					this option is not needed or supported.
				</p>
				...
				<p>
					...
					target directory, as set by the
					<code>
      ...
      --user user_name --password user_passwd
				...
				<p>
					...
					full backup, (defined by the
					<code>
						<a href="#-incremental-basedir">--incremental-basedir</a>
					</code>
					option) and creates delta files for these changes.
				</p>
				<p>
					In order to restore from a backup, you first need to run
					Mariabackup with the
					<code>
						<a href="#-prepare">--prepare</a>
					</code>
					command option, to make a full backup point-in-time consistent or
					to apply incremental backup deltas to base. Then you can run
					Mariabackup again with either the
					<code>
						<a href="#-copy-back">--copy-back</a>
					</code>
					or
				...
				<p>
					For more information, see <a
						href="../full-backup-and-restore-with-mariadb-backup/index">Full
						Backup and Restore</a> and <a
						href="../incremental-backup-and-restore-with-mariadb-backup/index">Incremental
				...
				<p>Defines how Mariabackup retrieves the binary log coordinates
					from the server.</p>
				<pre class="fixed" data-language="sql">--binlog-info[=OFF | ON | LOCKLESS | AUTO]
</pre>
				<p>
					The
					<code>--binlog-info</code>
					option supports the following retrieval methods. When no retrieval
				...
				<p>
					When enabled, whether using
					<code>ON</code>
					or
					<code>AUTO</code>
					, Mariabackup retrieves information from the binlog during the
					backup process. When disabled with
					<code>OFF</code>
					, Mariabackup runs without attempting to retrieve binary log
				...
				<pre class="fixed" data-language="sql">$ mariabackup --binlog-info --backup
</pre>
				<p>
					Currently, the
					<code>LOCKLESS</code>
					...
					<code>
						<a
							href="../files-created-by-mariabackup/index#xtrabackup_binlog_pos_innodb">xtrabackup_binlog_pos_innodb</a>
					</code>
				...
				<pre class="fixed" data-language="sql">$ mariabackup --close-files --prepare
</pre>
				...
				<p>
					...
					compression library to compress the stream, as described in <a
						href="../using-encryption-and-compression-tools-with-mariabackup/index">Using
						Encryption and Compression Tools With Mariabackup</a>.
				</p>
				<pre class="fixed" data-language="sql">--compress[=compression_algorithm]
</pre>
				<p>
					The
					<code>--compress</code>
					option supports the following algorithms. When no algorithm is
				...
				<pre class="fixed" data-language="sql">$ mariabackup --compress --backup
</pre>
				<p>
					To further configure backup compression, see the
					<code>
						<a href="#-compress-threads">--compress-threads</a>
					</code>
					and
					<code>
						<a href="#-compress-chunk-size">--compress-chunk-size</a>
					</code>
					options.
				</p>
				<p>
					...
					in the
					<code>
						<a href="../files-created-by-mariabackup/index#xtrabackup_info">xtrabackup_info</a>
					</code>
					file.
				</p>
				...
				<pre class="fixed" data-language="sql">--compress-chunk-size=#
</pre>
     ...
     --compress-threads=12 --compress-chunk-size=5M
				...
				<pre class="fixed" data-language="sql">--compress-threads=#
</pre>
				<p>Mariabackup can perform compression operations on the backup
				...
				<pre class="fixed" data-language="sql">$ mariabackup --compress --compress-threads=12 --backup
</pre>
				<p>
					To further configure backup compression, see the
					<code>
				...
				<p>
					...
					directory to the data directory, as defined by the
					<code>
						<a href="#-h-datadir">--datadir</a>
					</code>
					option. You must stop the MariaDB Server before running this
					command. The data directory must be empty. If you want to overwrite
					the data directory with the backup, use the
					<code>
						<a href="#-force-non-empty-directories">--force-non-empty-directories</a>
					</code>
					option.
				</p>
				<p>
					Bear in mind, before you can restore a backup, you first need to
					run Mariabackup with the
					<code>
						<a href="#-prepare">--prepare</a>
					</code>
					option. In the case of full backups, this makes the files
				...
				<pre class="fixed" data-language="sql">$ mariabackup --copy-back --force-non-empty-directories --backup
</pre>
				<p>
					Running the
					<code>--copy-back</code>
					command copies the backup files to the data directory. Use this
					command if you want to save the backup for later. If you don't want
					to save the backup for later, use the
					<code>
						<a href="#-move-back">--move-back</a>
					</code>
					command.
				...
				<pre class="fixed" data-language="sql">$ mariabackup --core-file --backup
</pre>
				<h3 class="anchored_heading" id="-databases">
					<code>--databases</code>
				</h3>
				<p>Defines the databases and tables you want to back up.</p>
				<pre class="fixed" data-language="sql">--databases="database[.table][ database[.table] ...]"
</pre>
      ...
      --databases="example.table1 example.table2"
				...
				<p>
					...
					databases or tables you don't want to back up using the
					<code>
						<a href="#-databases-exclude">--databases-exclude</a>
					</code>
					option.
				</p>
				<p>
					If a backup is a <a
						href="../partial-backup-and-restore-with-mariabackup/index">partial
						backup</a>, then Mariabackup will record that detail in the
					<code>
						<a href="../files-created-by-mariabackup/index#xtrabackup_info">xtrabackup_info</a>
					</code>
					file.
				...
				<p>
					...
					option, the
					<code>--databases</code>
					option can be used as described above, or it can be used to refer
					to a file, just as the
					<code>
						<a href="-databases-file">--databases-file</a>
					</code>
				...
				<pre class="fixed" data-language="sql">--databases-exclude="database[.table][ database[.table] ...]"
</pre>
				<p>Using this option, you can define the specific database or
      ...
      --databases-exclude="example.table1 example.table2"
				...
				<p>
					To include databases in the backup, see the
					<code>
				...
				<pre class="fixed" data-language="sql">--databases-file="/path/to/database-file"
</pre>
				<p>Format the databases file to list one element per line, with
					the following syntax:</p>
				<pre class="fixed" data-language="sql">database[.table]
</pre>
				<p>
					In cases where you need to back up a number of databases or
					specific tables in a database, you may find the syntax for the
					<code>
						<a href="#-databases">--databases</a>
					</code>
					and
					...
					options a little cumbersome. Using this option you can set the path
				...
				<p>
					...
					backup in a file called
					<code>main-backup</code>
					.
				</p>
...
$ mariabackup --backup --databases-file=main-backup
				...
				<pre class="fixed" data-language="sql">--datadir=PATH
</pre>
				<p>
					...
					up. It should be the same as the MariaDB Server
					<code>
						<a href="../server-system-variables/index#datadir">datadir</a>
					</code>
					system variable.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup -h /var/lib64/mysql
</pre>
				...
				<p>
					When you run Mariabackup with the
					<code>
						<a href="#-compress">--compress</a>
					</code>
					option, it compresses the subsequent backup files, using the
				...
				<pre class="fixed" data-language="sql">$ mariabackup --decompress
</pre>
				<p>
					You can enable the decryption of multiple files at a time using the
					<code>
						<a href="#-parallel">--parallel</a>
					</code>
					option. By default, Mariabackup does not remove the compressed
					files from the target directory. If you want to delete these files,
					use the
					<code>
						<a href="#-remove-original">--remove-original</a>
					</code>
					option.
				</p>
				<p>
				...
				<p>
					Defines the path to an extra default <a
						href="../configuring-mariadb-with-option-files/index">option
						file</a>.
				</p>
				<pre class="fixed" data-language="sql">--defaults-extra-file=/path/to/config
</pre>
				<p>
					Using this option, you can define an extra default <a
						href="../configuring-mariadb-with-option-files/index">option
						file</a> for Mariabackup. Unlike
					<code>
						<a href="#-defaults-file">--defaults-file</a>
					</code>
					, this file is read after the default <a
						href="../configuring-mariadb-with-option-files/index">option
						files</a> are read, allowing you to only overwrite the existing
					defaults.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
      ...
      --defaults-file=config.cnf
				...
				<p>
					Defines the path to the default <a
						href="../configuring-mariadb-with-option-files/index">option
						file</a>.
				</p>
				<pre class="fixed" data-language="sql">--defaults-file=/path/to/config
</pre>
				<p>
					Using this option, you can define a default <a
						href="../configuring-mariadb-with-option-files/index">option
						file</a> for Mariabackup. Unlike the
					<code>
						<a href="#-defaults-extra-file">--defaults-extra-file</a>
					</code>
					option, when this option is provided, it completely replaces all
					default <a href="../configuring-mariadb-with-option-files/index">option
						files</a>.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
     --defaults-file="config.cnf
				...
				<p>
					Defines the <a
						href="../configuring-mariadb-with-option-files/index#option-groups">option
						group</a> to read in the <a
						href="../configuring-mariadb-with-option-files/index">option
						file</a>.
				</p>
				<pre class="fixed" data-language="sql">--defaults-group="name"
</pre>
				<p>
...
compress_chunk_size = 64K
				...
				<p>
					Now whenever you run a backup with the
					<code>
						<a href="#-compress">--compress</a>
					</code>
					option, it always performs the compression using 12 threads and 64K
					chunks.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --compress --backup
</pre>
				<p>
					See <a href="../mariabackup-overview/index#server-option-groups">Mariabackup
						Overview: Server Option Groups</a> and <a
						href="../mariabackup-overview/index#client-option-groups">Mariabackup
						Overview: Client Option Groups</a> for a list of the option groups
					read by Mariabackup by default.
				</p>
				<h3 class="anchored_heading" id="-encrypted-backup">
					<code>--encrypted-backup</code>
				</h3>
				<p>
					When this option is used with
					<code>--backup</code>
					, if Mariabackup encounters a page that has a non-zero
					<code>key_version</code>
					value, then Mariabackup assumes that the page is encrypted.
				</p>
				<p>
					Use
					<code>--skip-encrypted-backup</code>
					instead to allow Mariabackup to copy unencrypted tables that were
					originally created before MySQL 5.1.48.
				</p>
				<p>
					This option was added in <a
						href="https://mariadb.com/kb/en/mariadb-10222-release-notes/">MariaDB
						10.2.22</a>, <a
						href="https://mariadb.com/kb/en/mariadb-10313-release-notes/">MariaDB
						10.3.13</a>, and <a
						href="https://mariadb.com/kb/en/mariadb-1042-release-notes/">MariaDB
						10.4.2</a>.
				</p>
				...
				<p>
					If this option is provided during the
					<code>--prepare</code>
					stage, then it tells Mariabackup to create
					<code>.cfg</code>
					files for each <a href="../innodb-file-per-table-tablespaces/index">InnoDB
						file-per-table tablespace</a>. These
					<code>.cfg</code>
					files are used to <a
						href="../innodb-file-per-table-tablespaces/index#copying-transportable-tablespaces">import
						transportable tablespaces</a> in the process of <a
						href="../partial-backup-and-restore-with-mariabackup/index">restoring
						partial backups</a> and <a
						href="../restoring-individual-tables-and-partitions-with-mariabackup/index">restoring
						individual tables and partitions</a>. .
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --prepare --export
</pre>
				<div class="mariadb_to_10_2_8 mariadb to_10_2_8 product">
					<strong class="product_title">MariaDB until <a
						href="https://mariadb.com/kb/en/mariadb-1028-release-notes/">10.2.8</a></strong>
					<p>
						In <a href="https://mariadb.com/kb/en/mariadb-1028-release-notes/">MariaDB
							10.2.8</a> and before, Mariabackup did not support the
						<code>
							<a href="index#-export">--export</a>
						</code>
						option. See <a href="https://jira.mariadb.org/browse/MDEV-13466">MDEV-13466</a>
						about that. In earlier versions of MariaDB, this means that
						Mariabackup could not create
						<code>.cfg</code>
						files for <a href="../innodb-file-per-table-tablespaces/index">InnoDB
							file-per-table tablespaces</a> during the
						<code>--prepare</code>
						stage. You can still <a
							href="../innodb-file-per-table-tablespaces/index#copying-transportable-tablespaces">import
							file-per-table tablespaces</a> without the
						<code>.cfg</code>
						files in many cases, so it may still be possible in those versions
						to <a href="../partial-backup-and-restore-with-mariabackup/index">restore
							partial backups</a> or to <a
							href="../restoring-individual-tables-and-partitions-with-mariabackup/index">restore
							individual tables and partitions</a> with just the
						<code>.ibd</code>
						files. If you have a <a
							href="../full-backup-and-restore-with-mariabackup/index">full
							backup</a> and you need to create
						<code>.cfg</code>
						files for <a href="../innodb-file-per-table-tablespaces/index">InnoDB
							file-per-table tablespaces</a>, then you can do so by preparing the
						backup as usual without the
						<code>--export</code>
						option, and then restoring the backup, and then starting the
						server. At that point, you can use the server's built-in features
						to <a
							href="../innodb-file-per-table-tablespaces/index#copying-transportable-tablespaces">copy
							the transportable tablespaces</a>.
					</p>
				</div>
				...
				<p>
					Saves an extra copy of the
					<code>
						<a
							href="../files-created-by-mariabackup/index#xtrabackup_checkpoints">xtrabackup_checkpoints</a>
					</code>
					and
					<code>
						<a href="../files-created-by-mariabackup/index#xtrabackup_info">xtrabackup_info</a>
				...
				<pre class="fixed" data-language="sql">--extra-lsndir=PATH
</pre>
				<p>
					When using the
					<code>
					...
					command option, Mariabackup produces a number of backup files in
					the target directory. Using this option, you can have Mariabackup
					produce additional copies of the
					<code>
						<a
							href="../files-created-by-mariabackup/index#xtrabackup_checkpoints">xtrabackup_checkpoints</a>
					</code>
				...
				<pre class="fixed" data-language="sql">$ mariabackup --extra-lsndir=extras/ --backup
</pre>
				...
				<p>
					When using Mariabackup with the
					<code>
						<a href="#-copy-back">--copy-back</a>
					</code>
					or
					...
					command options, they normally require a non-empty target directory
					to avoid conflicts. Using this option with either of command allows
					Mariabackup to use a non-empty directory.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --force-on-empty-directories --copy-back
</pre>
				...
				<pre class="fixed" data-language="sql">--ftwrl-wait-query-type=[ALL | UPDATE | SELECT]
</pre>
				<p>
					The
					<code>--ftwrl-wait-query-type</code>
					option supports the following query types. The default value is
					<code>ALL</code>
					.
				</p>
				<div class="cstm-style darkheader-nospace-borders">
					<table>
						...
						<tr>
							...
							<td>Waits until <code>
									<a href="../select/index">SELECT</a>
								</code> statements complete before issuing the global lock
							</td>
						</tr>
						<tr>
				...
				<p>
					...
					certain query types, such as for
					<code>
						<a href="../select/index">SELECT</a>
					</code>
					and
      ...
      --ftwrl-wait-query-type=UPDATE
				...
				<p>Defines the minimum threshold for identifying long-running
					queries for FTWRL.</p>
				<pre class="fixed" data-language="sql">--ftwrl-wait-threshold=#
</pre>
				<p>
					When Mariabackup runs, it issues a global lock to prevent data from
					...
					FTWRL. When it
					<code>
						<a href="#-ftwrl-wait-timeout">--ftwrl-wait-timeout</a>
					</code>
					is not 0 and a statement has run for at least the amount of time
					given this argument, Mariabackup waits until the statement
					completes or until the
					<code>
						<a href="#-ftwrl-wait-timeout">--ftwrl-wait-timeout</a>
					</code>
     ...
     --ftwrl-wait-threshold=30
				...
				<p>Defines the timeout to wait for FTWRL queries before setting
					the global lock.</p>
				<pre class="fixed" data-language="sql">--ftwrl-wait-timeout=#
</pre>
      ...
      --ftwrl-wait-timeout=5
				...
				<p>
					Defines whether you want to back up information about a <a
						href="../galera/index">Galera Cluster</a> node's state.
				</p>
				<p>
					When this option is used, Mariabackup creates an additional file
					called
					<code>
						<a
							href="../files-created-by-mariabackup/index#xtrabackup_galera_info">xtrabackup_galera_info</a>
					</code>
					, which records information about a <a href="../galera/index">Galera
						Cluster</a> node's state. It records the values of the
					<code>
						<a
							href="../galera-cluster-status-variables/index#wsrep_local_state_uuid">wsrep_local_state_uuid</a>
					</code>
					and
					<code>
						<a
							href="../galera-cluster-status-variables/index#wsrep_last_committed">wsrep_last_committed</a>
					</code>
					status variables.
				</p>
				<p>
					You should only use this option when backing up a <a
						href="../galera/index">Galera Cluster</a> node. If the server is
					not a <a href="../galera/index">Galera Cluster</a> node, then this
					option has no effect.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup --galera-info
</pre>
				...
				<p>
					...
					<code>PERCONA_SCHEMA.xtrabackup_history</code>
					table.
				</p>
				<pre class="fixed" data-language="sql">--history[=name]
</pre>
				<p>When using this option, Mariabackup records its operation in
					...
					analysis.</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup --history=backup_all
</pre>
				<p>
					Currently, the table it uses is named
					<code>PERCONA_SCHEMA.xtrabackup_history</code>
					, but expect that name to change in future releases. See <a
						href="https://jira.mariadb.org/browse/MDEV-19246">MDEV-19246</a>
					for more information.
				</p>
				<p>
					Mariabackup will also record this in the
					<code>
						<a href="../files-created-by-mariabackup/index#xtrabackup_info">xtrabackup_info</a>
					</code>
					file.
				...
				<pre class="fixed" data-language="sql">--host=name
</pre>
      ...
      --host="example.com"
				...
				<p>
					...
					names in databasename.tablename format. It is equivalent to the
					<code>
						<a href="#-tables">--tables</a>
					</code>
					option. This is only valid in
					<code>innobackupex</code>
					mode, which can be enabled with the
					<code>
						<a href="#-innobackupex">--innobackupex</a>
				...
				<pre class="fixed" data-language="sql">mariabackup --innobackupex --incremental
</pre>
				<p>
					...
					command option makes the operation incremental rather than a
					complete overwrite. When this option is specified, either the
					<code>
						<a href="#-incremental-lsn">--incremental-lsn</a> or
					</code>
					<a href="#-incremental-basedir">--incremental-basedir</a>
					<code> options can also be given. If neither option is
						given, option </code>
					<a href="#-incremental-basedir">--incremental-basedir</a>
					<code> is used by default, set to the first timestamped
						backup directory in the backup base directory.</code>
				</p>
     ...
     --target-dir=/data/backups
				...
				<p>Defines whether you want to take an incremental backup, based
					on another backup.</p>
				<pre class="fixed" data-language="sql">--incremental-basedir=PATH
</pre>
				<p>
					Using this option with the
					<code>
					...
					command option makes the operation incremental rather than a
					complete overwrite. Mariabackup will only copy pages from
					<code>.ibd</code>
     ...
     --target-dir=/data/backups
				...
				<p>Defines whether you want to take an incremental backup, based
					on another backup.</p>
				<pre class="fixed" data-language="sql">--increment-dir=PATH
</pre>
				<p>
					...
					command option makes the operation incremental rather than a
					complete overwrite. Mariabackup will apply
					<code>.delta</code>
      ...
      --increment-dir=backups/
				...
				<p>
					...
					being backed up, even when there's bitmap data on the changes. <a
						href="../what-is-mariadb-102/index">MariaDB 10.2</a> and later
					does not support changed page bitmaps, so this option is useless in
					those versions. See <a
						href="https://jira.mariadb.org/browse/MDEV-18985">MDEV-18985</a>
					for more information.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
     ...
     --incremental-force-scan
				...
				<pre class="fixed" data-language="sql">--incremental-history-name=name
</pre>
     ...
     --incremental-history-name=morning_backup
				...
				<pre class="fixed" data-language="sql">--incremental-history-uuid=name
</pre>
				<p>
					...
					identifying a previous backup to increment from. It checks
					<code>
						<a href="#-incremental-history-name">--incremental-history-name</a>
					</code>
					,
					<code>
						<a href="#-incremental-basedir">--incremental-basedir</a>
					...
					, and
					<code>
						<a href="#-incremental-lsn">--incremental-lsn</a>
					</code>
      ...
      --incremental-history-uuid=main-backup012345678
				...
				<pre class="fixed" data-language="sql">--incremental-lsn=name
</pre>
				<p>
					...
					operations. During backups, Mariabackup only copies
					<code>.ibd</code>
				...
				<pre class="fixed" data-language="sql">$ mariabackup --innobackupex
</pre>
				<p>
					In
					<code>innobackupex</code>
				...
				<ul start="1">
					<li>To prepare a backup, the <code>
							<a href="#-apply-log">--apply-log</a>
						</code> option is used instead of the <code>
							<a href="#-prepare">--prepare</a>
						</code> option.
					</li>
					<li>To create an <a
						...
						</code> option is supported.
					</li>
					<li>The <code>
							<a href="#-no-timestamp">--no-timestamp</a>
						</code> option is supported.
					</li>
					<li>To create a <a
						href="../partial-backup-and-restore-with-mariabackup/index">partial
					...
					<li>To create a <a
						href="../partial-backup-and-restore-with-mariabackup/index">partial
							backup</a>, the <code>
							<a href="#--databases">--databases</a>
						</code> option can still be used, but it's behavior changes slightly.
					</li>
					<li>The <code>
							<a href="#-target-dir">--target-dir</a>
						</code> option is not used to specify the backup directory. The backup
				...
				<p>
					...
					<code>innobackupex</code>
					mode is to allow scripts and tools to more easily migrate to
					Mariabackup if they were originally designed to use the
					<code>innobackupex</code>
					utility that is included with <a
						href="../percona-xtrabackup-overview/index">Percona XtraBackup</a>.
					It is not recommended to use this mode in new scripts, since it is
					not guaranteed to be supported forever. See <a
						href="https://jira.mariadb.org/browse/MDEV-20552">MDEV-20552</a>
					for more information.
				</p>
				...
				<p>
					...
					want to disable it, use
					<code>
						<a href="#-skip-innodb-adaptive-hash-index">--skip-innodb-adaptive-hash-index</a>
					</code>
					.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
      --innodb-adaptive-hash-index
				...
				<pre class="fixed" data-language="sql">--innodb-autoextend-increment=36
</pre>
     ...
     --innodb-autoextend-increment=35
				...
				<pre class="fixed" data-language="sql">--innodb-buffer-pool-size=124M
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
      ...
      --innodb-buffer-pool-size=124M
				...
				<p>
					...
					<code>CRC32</code>
					,
					<code>STRICT_CRC32</code>
					,
					<code>INNODB</code>
					,
					<code>STRICT_INNODB</code>
					,
					<code>NONE</code>
					,
					<code>STRICT_NONE</code>
					.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
      ---innodb-checksum-algorithm=strict_innodb
				...
				<pre class="fixed" data-language="sql">--innodb-data-file-path=/path/to/file
</pre>
				<p>
					Mariabackup initializes its own embedded instance of InnoDB using
					the same configuration as defined in the configuration file. Using
					...
					is appended to the
					<code>
						<a href="#-innodb-data-home-dir">--innodb-data-home-dir</a>
					</code>
					option.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
     ...
     --innodb-data-home-dir=/var/dbs/mysql/data
				...
				<pre class="fixed" data-language="sql">--innodb-data-home-dir=PATH
</pre>
				<p>
					Mariabackup initializes its own embedded instance of InnoDB using
					the same configuration as defined in the configuration file. Using
					this option you can define the path to the directory containing
					InnoDB data files. You can specific the files using the
					<code>
						<a href="#-innodb-data-file-path">--innodb-data-file-path</a>
					</code>
					option.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
     ...
     --innodb-data-home-dir=/var/dbs/mysql/data
				...
				<p>
					...
					doublewrites, use the
					<code>
						<a href="#-skip-innodb-doublewrite">--skip-innodb-doublewrite</a>
					</code>
					option.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
     --innodb-doublewrite
				...
				<pre class="fixed" data-language="sql">--innodb-file-io-threads=#
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
     ...
     --innodb-file-io-threads=5
      ...
      --innodb-flush-method==_DIRECT_NO_FSYNC
				...
				<p>
					Note, the
					<code>0_DIRECT_NO_FSYNC</code>
					method is only available with <a
						href="../what-is-mariadb-100/index">MariaDB 10.0</a> and later.
					The
					<code>ALL_O_DIRECT</code>
					method available with version 5.5 and later, but only with tables
					using the XtraDB storage engine.
				</p>
				...
				<pre class="fixed" data-language="sql">--innodb-io-capacity=#
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
     ...
     --innodb-io-capacity=200
				...
				<p>
					...
					disable it, use the
					<code>
						<a href="#-skip-innodb-log-checksums">--skip-innodb-log-checksums</a>
					</code>
					option.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
      --innodb-log-checksums
				...
				<pre class="fixed" data-language="sql">--innodb-log-group-home-dir=PATH
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
     ...
     --innodb-log-group-home-dir=/path/to/logs
				...
				<p>Defines the percentage of dirty pages allowed in the InnoDB
					buffer pool.</p>
				<pre class="fixed" data-language="sql">--innodb-max-dirty-pages-pct=#
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
     ...
     --innodb-max-dirty-pages-pct=80
				...
				<pre class="fixed" data-language="sql">--innodb-open-files=#
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
      ...
      --innodb-open-files=10
				...
				<pre class="fixed" data-language="sql">--innodb-page-size=#
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
     ...
     --innodb-page-size=16k
				...
				<pre class="fixed" data-language="sql">--innodb-read-io-threads=#
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
      ...
      --innodb-read-io-threads=4
				...
				<pre class="fixed" data-language="sql">--innodb-undo-directory=PATH
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
     ...
     --innodb-undo-directory=/path/to/innodb_undo
				...
				<pre class="fixed" data-language="sql">--innodb-undo-tablespaces=#
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
      ...
      --innodb-use-native-aio
				...
				<pre class="fixed" data-language="sql">--innodb-write-io-threads=#
</pre>
				<p>Mariabackup initializes its own embedded instance of InnoDB
					using the same configuration as defined in the configuration file.
     ...
     --innodb-write-io-threads=4
				...
				<pre class="fixed" data-language="sql">--kill-long-queries-timeout=#
</pre>
				<p>
      ...
      --kill-long-queries-timeout=10
				...
				<p>Defines the query type the utility can kill to unblock the
					global lock.</p>
				<pre class="fixed" data-language="sql">--kill-long-query-type=ALL | UPDATE | SELECT
</pre>
				<p>
					...
					Using this option, you can choose the types of query it kills:
					<code>
						<a href="../select/index">SELECT</a>
					</code>
					,
      ...
      --kill-long-query-type=UPDATE
				...
				<p>
					...
					<a href="https://mariadb.com/kb/en/mariadb-1029-release-notes/">MariaDB
						10.2.9</a> and later.
				</p>
				...
				<pre class="fixed" data-language="sql">--log-bin[=name]
</pre>
				...
				<p>Defines the copy interval between checks done by the log
					copying thread.</p>
				<pre class="fixed" data-language="sql">--log-copy-interval=#
</pre>
      ...
      --datadir=/var/mysql
				...
				<p>
					...
					want to save the backup for later, use the
					<code>
						<a href="#-copy-back">--copy-back</a>
					</code>
					command.
				...
				<p>
					This option may eventually be removed. See <a
						href="https://jira.mariadb.org/browse/MDEV-19753">MDEV-19753</a>
					for more information.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup --no-backup-locks
</pre>
				<h3 class="anchored_heading" id="-no-lock">
					<code>--no-lock</code>
				</h3>
				<p>
					Disables table locks with the
					<code>FLUSH TABLE WITH READ LOCK</code>
					statement.
				</p>
				<p>
				...
				<ul start="1">
					...
					<li>You <strong>do not care</strong> if the binary log
						position included with the backup in <code>
							<a
								href="../files-created-by-mariabackup/index#xtrabackup_binlog_info">xtrabackup_binlog_info</a>
						</code> is consistent with the data.
					</li>
					<li><strong>All</strong> tables you're backing up use the
						InnoDB storage engine.</li>
				</ul>
				<pre class="fixed" data-language="sql">$ mariabackup --backup --no-lock
</pre>
				<p>
					If you're considering
					<code>--no-lock</code>
					due to backups failing to acquire locks, this may be due to
					incoming replication events preventing the lock. Consider using the
					<code>
						<a href="#-safe-slave-backup">--safe-slave-backup</a>
					</code>
					option to momentarily stop the replication slave thread. This
					alternative may help the backup to succeed without resorting to
					<code>--no-lock</code>
					.
				</p>
				...
				<p>
					...
					you would like to enable the version check, use the
					<code>
						<a href="#-version-check">--version-check</a>
					</code>
					option.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup --no-version-check
</pre>
				...
				<pre class="fixed" data-language="sql">--open-file-limit=#
</pre>
				<p>
					Using this option, you can define the maximum number of file
					descriptors Mariabackup reserves with
					<code>setrlimit()</code>
					.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
      --open-file-limit=
				...
				<p>Defines the number of threads to use for parallel data file
					transfer.</p>
				<pre class="fixed" data-language="sql">--parallel=#
</pre>
				...
				<pre class="fixed" data-language="sql">--password=passwd
</pre>
				<p>
      ...
      --password=root_password
				...
				<pre class="fixed" data-language="sql">--plugin-dir=PATH
</pre>
				<p>
					Using this option, you can define the path Mariabackup reads for
					MariaDB Server plugins. It only uses it during the
					<code>
						<a href="#-prepare">--prepare</a>
					</code>
					phase to load the encryption plugin. It defaults to the
					<code>
						<a href="../server-system-variables/index#plugin_dir">plugin_dir</a>
					</code>
      ...
      --plugin-dir=/var/mysql/lib/plugin
				...
				<pre class="fixed" data-language="sql">--plugin-load=name
</pre>
				<p>
					Using this option, you can define the encryption plugin you want to
					load. It is only used during the
					<code>
						<a href="#-prepare">--prepare</a>
					</code>
					phase to load the encryption plugin. It defaults to the server
					<code>--plugin-load</code>
					option.
				</p>
				<p>
					The option was removed starting from <a
						href="https://mariadb.com/kb/en/mariadb-10218-release-notes/">MariaDB
						10.2.18</a>
				</p>
				...
				<pre class="fixed" data-language="sql">--port=#
</pre>
				<p>
					When you run Mariabackup, it connects to MariaDB Server in order to
					...
					TCP/IP. To set the host, see the
					<code>
						<a href="#-h-host">--host</a>
					</code>
					option. Use
      ...
      --port=3306
				...
				<pre class="fixed" data-language="sql">$ mariabackup --print-defaults
</pre>
				...
				<p>
					...
					options that the utility requires to run the
					<code>
						<a href="#-copy-back">--copy-back</a>
					</code>
					command option.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --print-param
</pre>
				...
				<pre class="fixed" data-language="sql">$ mariabackup --backup --rsync
</pre>
				<p>
				...
				<p>
					...
					it stops slave SQL threads and waits until the
					<code>Slave_open_temp_tables</code>
					in the
					<code>SHOW STATUS</code>
					statement is zero. If there are no open temporary tables, the
					backup runs, otherwise the SQL thread starts and stops until there
      ...
      --safe-slave-backup-timeout=500
				...
				<p>
					The backup fails if the
					<code>Slave_open_temp_tables</code>
					doesn't reach zero after the timeout period set by the
					<code>
						<a href="#-safe-slave-backup-timeout">--safe-slave-backup-timeout</a>
					</code>
					option.
				</p>
				...
				<pre class="fixed" data-language="sql">--safe-slave-backup-timeout=#
</pre>
				<p>
					When running Mariabackup on a server that uses replication, you may
					occasionally encounter locks that block backups. With the
					<code>
						<a href="#-safe-slave-backup">--safe-slave-backup</a>
					</code>
					option, it waits until the
					<code>Slave_open_temp_tables</code>
					in the
					<code>SHOW STATUS</code>
					statement reaches zero. Using this option, you set how long it
					waits. It defaults to 300.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
      --safe-slave-backup \
      --safe-slave-backup-timeout=500
				...
				<p>
					...
					before 4.1.1. This feature is enabled by default. Use the
					<code>
						<a href="#-skip-secure-auth">--skip-secure-auth</a>
					</code>
					option to disable it.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup --secure-auth
</pre>
				...
				<p>
					...
					Index. This feature is enabled by default for Mariabackup. If you
					want to explicitly enable it, use
					<code>
						<a href="#-innodb-adaptive-hash-index">--innodb-adaptive-hash-index</a>
					</code>
					.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
      --skip-innodb-adaptive-hash-index
				...
				<p>
					...
					enable doublewrites, use the
					<code>
						<a href="#-innodb-doublewrite">--innodb-doublewrite</a>
					</code>
					option.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
     --skip-innodb-doublewrite
				...
				<p>
					...
					enable it, use the
					<code>
						<a href="#-innodb-log-checksums">--innodb-log-checksums</a>
					</code>
					option.
				</p>
				...
				<p>
					...
					default, it refuses these connections. Use the
					<code>
						<a href="#-secure-auth">--secure-auth</a>
					</code>
					option to explicitly enable it.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup --skip-secure-auth
</pre>
				...
				<p>Prints the binary log position and the name of the master
					server.</p>
				<p>
					If the server is a <a href="../replication/index">replication
						slave</a>, then this option causes Mariabackup to print the hostname
					of the slave's replication master and the <a
						href="../binary-log/index">binary log</a> file and position of the
					<a href="../replication-threads/index#slave-sql-thread">slave's
						SQL thread</a> to
					<code>stdout</code>
					.
				</p>
				<p>
					This option also causes Mariabackup to record this information as a
					<code>
						<a href="../change-master-to/index">CHANGE MASTER</a>
					</code>
					command that can be used to set up a new server as a slave of the
					original server's master after the backup has been restored. This
					information will be written to to the
					<code>
						<a
							href="../files-created-by-mariabackup/index#xtrabackup_slave_info">xtrabackup_slave_info</a>
					</code>
					file.
				</p>
				<p>
					Mariabackup does <strong>not</strong> check if <a
						href="../gtid/index">GTIDs</a> are being used in replication. It
					takes a shortcut and assumes that if the
					<code>
						<a href="../gtid/index#gtid_slave_pos">gtid_slave_pos</a>
					</code>
					system variable is non-empty, then it writes the
					<code>
						<a href="../change-master-to/index">CHANGE MASTER</a>
					</code>
					command with the
					<code>
						<a href="../change-master-to/index#master_use_gtid">MASTER_USE_GTID</a>
					</code>
					option set to
					<code>slave_pos</code>
					...
					command with the
					<code>
						<a href="../change-master-to/index#master_log_file">MASTER_LOG_FILE</a>
					</code>
					and
					<code>
						<a href="../change-master-to/index#master_log_pos">MASTER_LOG_POS</a>
					</code>
					options using the master's <a href="../binary-log/index">binary
						log</a> file and position. See <a
						href="https://jira.mariadb.org/browse/MDEV-19264">MDEV-19264</a>
					for more information.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --slave-info
</pre>
				<h3 class="anchored_heading" id="-s-socket">
					<code>-S, --socket</code>
				</h3>
				<p>Defines the socket for connecting to local database.</p>
				<pre class="fixed" data-language="sql">--socket=name
</pre>
				<p>
					...
					accepts a string argument. For more information, see the
					<code>mysql --help</code>
					command.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
      --socket=/var/mysql/mysql.sock
				...
				<p>
					Enables <a href="../data-in-transit-encryption/index">TLS</a>. By
					using this option, you can explicitly configure Mariabackup to to
					encrypt its connection with <a
						href="../data-in-transit-encryption/index">TLS</a> when
					communicating with the server. You may find this useful when
				...
				<ul start="1">
					<li><code>
							<a href="#-ssl-ca">--ssl-ca</a>
						</code></li>
					<li><code>
							<a href="#-ssl-capath">--ssl-capath</a>
						</code></li>
					<li><code>
							<a href="#-ssl-cert">--ssl-cert</a>
						</code></li>
					<li><code>
							<a href="#-ssl-cipher">--ssl-cipher</a>
						</code></li>
					<li><code>
							<a href="#-ssl-key">--ssl-key</a>
						</code></li>
				</ul>
				...
				<p>
					...
					certificates for trusted Certificate Authorities (CAs) to use for <a
						href="../data-in-transit-encryption/index">TLS</a>. This option
					requires that you use the absolute path, not a relative path. For
					example:
				</p>
				<pre class="fixed" data-language="sql">--ssl-ca=/etc/my.cnf.d/certificates/ca.pem
</pre>
   ...
   --ssl-ca=/etc/my.cnf.d/certificates/ca.pem 
				...
				<p>
					See <a
						href="../secure-connections-overview/index#certificate-authorities-cas">Secure
						Connections Overview: Certificate Authorities (CAs)</a> for more
					information.
				</p>
				<p>
				...
				<p>
					...
					Certificate Authority (CA) to use for <a
						href="../data-in-transit-encryption/index">TLS</a>. This option
					requires that you use the absolute path, not a relative path. For
					example:
				</p>
				<pre class="fixed" data-language="sql">--ssl-capath=/etc/my.cnf.d/certificates/ca/
</pre>
				<p>This option is usually used with other TLS options. For
					example:</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
   ...
   --ssl-capath=/etc/my.cnf.d/certificates/ca/
				...
				<p>
					The directory specified by this option needs to be run through the
					<code>
						<a href="https://www.openssl.org/docs/man1.1.1/man1/rehash.html">openssl
							rehash</a>
					</code>
					command.
				</p>
				...
				<p>
					Defines a path to the X509 certificate file to use for <a
						href="../data-in-transit-encryption/index">TLS</a>. This option
					requires that you use the absolute path, not a relative path. For
					example:
				</p>
				<pre class="fixed" data-language="sql">--ssl-cert=/etc/my.cnf.d/certificates/client-cert.pem
</pre>
				<p>This option is usually used with other TLS options. For
					example:</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
   ...
   --ssl-ca=/etc/my.cnf.d/certificates/ca.pem
				...
				<p>
					Defines the list of permitted ciphers or cipher suites to use for <a
						href="../data-in-transit-encryption/index">TLS</a>. For example:
				</p>
				<pre class="fixed" data-language="sql">--ssl-cipher=name
</pre>
				<p>This option is usually used with other TLS options. For
					example:</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
   ...
   --ssl-cipher=TLSv1.2
				...
				<p>
					To determine if the server restricts clients to specific ciphers,
					check the
					<code>
						<a href="../ssltls-system-variables/index#ssl_cipher">ssl_cipher</a>
					</code>
					system variable.
				</p>
				<p>
				...
				<p>
					...
					revoked X509 certificates to use for <a
						href="../data-in-transit-encryption/index">TLS</a>. This option
					requires that you use the absolute path, not a relative path. For
					example:
				</p>
				<pre class="fixed" data-language="sql">--ssl-crl=/etc/my.cnf.d/certificates/crl.pem
</pre>
				<p>This option is usually used with other TLS options. For
					example:</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
   ...
   --ssl-crl=/etc/my.cnf.d/certificates/crl.pem
				...
				<p>
					See <a
						href="../secure-connections-overview/index#certificate-revocation-lists-crls">Secure
						Connections Overview: Certificate Revocation Lists (CRLs)</a> for more
					information.
				</p>
				<p>
					...
					not supported. See <a
						href="../tls-and-cryptography-libraries-used-by-mariadb/index">TLS
						and Cryptography Libraries Used by MariaDB</a> for more information
				...
				<p>
					Defines a path to a directory that contains one or more PEM files
					that should each contain one revoked X509 certificate to use for <a
						href="../data-in-transit-encryption/index">TLS</a>. This option
					requires that you use the absolute path, not a relative path. For
					example:
				</p>
				<pre class="fixed" data-language="sql">--ssl-crlpath=/etc/my.cnf.d/certificates/crl/
</pre>
				<p>This option is usually used with other TLS options. For
					example:</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
   ...
   --ssl-crlpath=/etc/my.cnf.d/certificates/crl/ 
				...
				<p>
					Defines a path to a private key file to use for <a
						href="../data-in-transit-encryption/index">TLS</a>. This option
					requires that you use the absolute path, not a relative path. For
					example:
				</p>
				<pre class="fixed" data-language="sql">--ssl-key=/etc/my.cnf.d/certificates/client-key.pem
</pre>
				<p>This option is usually used with other TLS options. For
					example:</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
   ...
   --ssl-ca=/etc/my.cnf.d/certificates/ca.pem
				...
				<p>
					Enables <a
						href="../secure-connections-overview/index#server-certificate-verification">server
						certificate verification</a>. This option is disabled by default.
				</p>
				<p>This option is usually used with other TLS options. For
					example:</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
   ...
   --ssl-verify-server-cert
				...
				<pre class="fixed" data-language="sql">--stream=xbstream
</pre>
				<p>
				...
				<pre class="fixed" data-language="sql">$ mariabackup --stream=xbstream &gt; backup.xb
</pre>
				<p>
					To extract all files from the xbstream archive into a directory use
					the
					<code>mbstream</code>
					utility
				</p>
				<pre class="fixed" data-language="sql">$ mbstream  -x &lt; backup.xb
</pre>
				<p>
					If a backup is streamed, then Mariabackup will record the format in
					the
					<code>
						<a href="../files-created-by-mariabackup/index#xtrabackup_info">xtrabackup_info</a>
					</code>
				...
				<pre class="fixed" data-language="sql">--tables=REGEX
</pre>
				<p>
					...
					the backup, see the
					<code>
						<a href="#-tables-exclude">--tables-exclude</a>
					</code>
					option.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
     ...
     --tables-exclude=nodes_tmp
				...
				<pre class="fixed" data-language="sql">--tables-exclude=REGEX
</pre>
				<p>
					Using this option, you can define what tables you want Mariabackup
     ...
     --tables-exclude=nodes_tmp
				...
				<pre class="fixed" data-language="sql">--tables-file=/path/to/file
</pre>
				<p>
     ...
     --tables-file=/etc/mysql/backup-file
				...
				<pre class="fixed" data-language="sql">--target-dir=/path/to/target
</pre>
       ...
       --target-dir=/data/backups
				...
				<p>Defines the limit for I/O operations per second in IOS
					values.</p>
				<pre class="fixed" data-language="sql">--throttle=#
</pre>
				<p>
				...
				<pre class="fixed" data-language="sql">--tls-version="TLSv1.2,TLSv1.3"
</pre>
				<p>This option is usually used with other TLS options. For
					example:</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
   ...
   --tls-version="TLSv1.2,TLSv1.3"
				...
				<p>
					This option was added in <a
						href="https://mariadb.com/kb/en/mariadb-1046-release-notes/">MariaDB
						10.4.6</a>.
				</p>
				<p>
					See <a
						href="../secure-connections-overview/index#tls-protocol-versions">Secure
						Connections Overview: TLS Protocol Versions</a> for more information.
				</p>
				<h3 class="anchored_heading" id="-t-tmpdir">
					<code>-t, --tmpdir</code>
				</h3>
				<p>Defines path for temporary files.</p>
				<pre class="fixed" data-language="sql">--tmpdir=/path/tmp[;/path/tmp...]
</pre>
				<p>
					Using this option, you can define the path to a directory
					Mariabackup uses in writing temporary files. If you want to use
					more than one, separate the values by a semicolon (that is,
					<code>;</code>
					). When passing multiple temporary directories, it cycles through
     ...
     --tmpdir=/data/tmp;/tmp
				...
				<pre class="fixed" data-language="sql">--use-memory=124M
</pre>
				<p>
					Using this option, you can define the buffer pool size for
					Mariabackup. Use it instead of
					<code>buffer_pool_size</code>
					.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup \
      --use-memory=124M
				...
				<pre class="fixed" data-language="sql">--user=name
-u name
</pre>
      ...
      --password=root_passwd
				...
				<pre class="fixed" data-language="sql">$ mariabackup --version
</pre>
				...
				<p>
					...
					would like to disable the version check, use the
					<code>
						<a href="#-no-version-check">--no-version-check</a>
					</code>
					option.
				</p>
				<pre class="fixed" data-language="sql">$ mariabackup --backup --version-check
</pre>
			</div>
		</div>
